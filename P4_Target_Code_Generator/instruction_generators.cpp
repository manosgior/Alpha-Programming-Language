...
void generate(TCodeOpcode op, quad& q) {
    Instruction t = Instruction(op, VMarg(q.result), VMarg(q.arg1), (q.arg2 != nullptr ? VMarg(q.arg2) : VMarg()), q.line);
...
void generate_FUNCSTART(quad& q) {
    ...
    instruction_table.emit(t);
    instruction_table.addUserFunc(q.result->getSymbol()->getFuncAddress(), q.result->getSymbol()->getTotalLocals(), q.result->getName());
}
