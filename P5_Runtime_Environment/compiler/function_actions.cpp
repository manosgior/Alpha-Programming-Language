...
expr * Manage_funcdef_N_funcprefix_LEFT_PAREN_idlist_RIGHT_PAREN_block(unsigned N, char * funcprefix, stmtInfo * block) {
    ...
    symbol_table.setFunctionTotalFormals(std::string(funcprefix), symbol_table.getFormalsOffset());
    ...
    quad_table.getQuad(N + 1).result->getSymbol()->setTotalLocals( symbol_table.getFunctionLocalOffset() );
    quad_table.getQuad(N + 1).result->getSymbol()->setTotalFormals( symbol_table.getFormalsOffset() );
    return toReturn;
}
...
expr * Manage_term_PLUS_PLUS_lvalue(expr * lvalue) {
    ...
    if (lvalueType == SymTable_Entry_Type::user_function ||
        ...
        if (lvalue->getType() == tableitem_e) {
            ...
            //arg1 is index, arg2 is value, res is table
            quad_table.emit(iopcode::tablesetelem, lvalue->getIndex(), toReturn, lvalue, 0, yylineno);
        } else {
...
expr * Manage_term_lvalue_PLUS_PLUS(expr * lvalue) {
    ...
    if (lvalueType == SymTable_Entry_Type::user_function ||
        ...
        if (lvalue->getType() == tableitem_e) {
            ...
            //arg1 is index, arg2 is value, res is table
            quad_table.emit(iopcode::tablesetelem, lvalue->getIndex(), value, lvalue, 0, yylineno);
        } else {
            quad_table.emit(iopcode::assign, lvalue, nullptr, toReturn, 0, yylineno);
            quad_table.emit(iopcode::add, lvalue, expr::newexpr_constnum(1), lvalue, 0, yylineno);
        }
...
expr * Manage_assignexpr_lvalue_ASSIGNMENT_expr(expr * lvalue, expr * expression) {
    ...
    if (lvalue->getType() == expr_t::tableitem_e) {
        ...
        if(tmp_e->getType() != expr_t::nil_e){
            toReturn = expr::emit_iftableitem(lvalue);
            toReturn->setType(expr_t::assignexpr_e);
        }
        else
            toReturn = expr::newexpr_constnil();
    } else {
...
stmtInfo* Manage_while_whilestart_whilecond_stmt(unsigned int label, unsigned int toPatch, stmtInfo* stmt) {
...
stmtInfo* Manage_forstmt_forprefix_N_elist_RIGHT_PAREN_N_stmt_N(unsignedPair* forprefix, unsigned N1, unsigned N2, stmtInfo* stmt, unsigned N3) {
...
stmtInfo* Manage_stmt_forstmt(stmtInfo* forstmtLists) {
    std::get<2>(*forstmtLists) = false;
    return forstmtLists;
}

stmtInfo* Manage_stmt_whilestmt(stmtInfo* whilestmtLists) {
    std::get<2>(*whilestmtLists) = false;
    return whilestmtLists;
}
