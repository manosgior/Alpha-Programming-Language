...
void InstructionTable::exportByteCode(std::ofstream& os){
    ...
    for (userFuncInfo i : userFuncs) {
        ...
        os.write((char *)&(i.numberOfFormals), sizeof(unsigned));
        tmp = i.id.size()+1;
        os.write((char*)&tmp, sizeof(size_t));
    ...
    os.write((char *) &tmp, sizeof(size_t));
    size_t numOfGlobals = symbol_table.getTotalGlobals();
    os.write((char *) &numOfGlobals, sizeof(size_t));
...
void InstructionTable::addUserFunc(unsigned address, unsigned localsize, unsigned numberOfFormals, std::string id) {
    userFuncInfo tmp = {address, localsize, numberOfFormals, id};
