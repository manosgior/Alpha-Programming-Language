...
SymbolInfo::SymbolInfo(const SymbolInfo &other){
    ...
    this->totalFormals = other.totalFormals;
...
size_t SymbolInfo::getTotalFormals(){
    return totalFormals;
}

void SymbolInfo::setTotalFormals(size_t f){
    totalFormals = f;
}

void SymbolInfo::setScopespaceType(Scopespace_Type s){
    scopeType = s;
}

Scopespace_Type SymbolInfo::getScopespaceType(){
    return scopeType;
}
...
SymbolInfo SymTable::Insert(std::string key, SymTable_Entry_Type type, size_t line, size_t scope) {
    ...
    if(type == formal_argument)
        info.setScopespaceType(formalarg);
    else if( type == global_variable || (this->inSpecialScope(function_scope) == -1 && type == local_variable) )
        info.setScopespaceType(programvar);
    else
        info.setScopespaceType(functionlocal);
    ...
    if(info.getScopespaceType() == programvar)
        totalGlobals++;
...
void SymTable::setFunctionTotalFormals(std::string key, size_t totalFormals){
    std::map<std::string, SymTable_InfoStorage *>::iterator it = name2info_table.find(key);
    if (it == name2info_table.end()) {
        return;
    } else {
        std::vector<SymbolInfo>& actVec = it->second->getActiveVector();
        actVec.back().setTotalFormals(totalFormals);
    }    
}
...
size_t SymTable::getFormalsOffset() {
    return lastPoppedOffsets.getFormalArgOffset();
}
...
size_t SymTable::getTotalGlobals(){
    return totalGlobals;
}